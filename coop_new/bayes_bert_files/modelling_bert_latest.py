...
class CooperativeLinear(nn.Linear):
    def __init__(
        ...
        **kwargs
        ...
        self.cons_scale = 1
        #self.logvar = nn.Parameter(torch.rand(out_features,out_features))
    ...
    def forward(self, x):        
        ...
        # var = torch.diag(self.logvar.exp() * self.scale.to(x.device) * self.cons_scale)
    ...
    def return_variational_loss(self):
...
class BertModel(BertPreTrainedModel):
    ...
    def get_variational_loss(self):
        ...
        for layer in range(self.config.num_hidden_layers):
            if 'output' in self.config.expert_locations.split(','):
                loss += self.encoder.layer[layer].output.get_variational_loss()
