...
class TransformerEncoderLayerBase(nn.Module):
    ...
    *cfg.encoder.normalize_before* to ``True``.
    ...
    def __init__(self, cfg, return_fc=False):
        super().__init__()
        self.cfg = cfg
        #self.num_experts = 4 #cfg.num_experts
        self.cooperative_modules = cfg.cooperative_modules
    ...
    def build_fc1(self, input_dim, output_dim, q_noise, qn_block_size):
        if 'intermediate' in self.cooperative_modules:
            return quant_noise(
                CooperativeLinear(input_dim, output_dim, self.num_experts), p=q_noise, block_size=qn_block_size
            )
        else:
            return quant_noise(
    ...
    def build_fc2(self, input_dim, output_dim, q_noise, qn_block_size):
        if 'output' in self.cooperative_modules:
            return quant_noise(
                CooperativeLinear(input_dim, output_dim, self.num_experts), p=q_noise, block_size=qn_block_size
            )
        else:
    ...
    def build_self_attention(self, embed_dim, cfg):
        return MultiheadAttention(
            ...
            cooperative_modules=self.cooperative_modules,
