...
class LanguageModelingConfig(FairseqDataclass):
    ...
    cooperative_modules: str = field(
        default='query, key, value, attention_out, intermediate, output',
        metadata={"help": "modules where cooperative linear is added"},
    )

    use_entropy: bool = field(
        default=False,
        metadata={'help': "Whether to use entropy loss"}
    )
...
class LanguageModelingTask(LegacyFairseqTask):
    ...
    def _get_loss(self, sample, model, criterion):
        ...
        return loss, logits, logging_output
    ...
    def train_step(
        ...
        with torch.autograd.profiler.record_function("forward"):
            with torch.cuda.amp.autocast(enabled=(isinstance(optimizer, AMPOptimizer))):
                loss1, logits1, logging_output1 = self._get_loss(sample, model, criterion)
        ...
        if self.args.use_entropy == True:
            ...
            for module in list(dict(model.named_modules()).values()):
                if type(module).__name__ == 'CooperativeLinear' or type(module).__name__ == 'CooperativeConv1D':
                    loss2 += module.get_variational_loss()
            loss = loss1 + loss2
        else:
            loss = loss1
        #loss = loss1
